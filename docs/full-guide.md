...
> 聚合通知发送到企业微信时，系统会在现有仪表盘文本/Markdown 截图之后，额外尝试发送一张“最终总结表格图”。该附加图片依赖 `matplotlib` 和可用中文字体；Linux 环境建议安装 `fonts-noto-cjk` 或 `fonts-wqy-zenhei`。表格中的较长“建议 / 趋势”文本会在单元格内自动换行，并按内容自适应增高行高。若运行环境缺失依赖或字体，系统会仅跳过这张附加图片，不影响原有企业微信主消息发送。
...
1. 创建 Slack App：[https://api.slack.com/apps](https://api.slack.com/apps) → Create New App
