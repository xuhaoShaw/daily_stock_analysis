...
- [新功能] 新增市场推荐功能：可从 A 股/ETF 实时活跃行情中发现候选标的，按涨跌幅、成交额、量比和风险规则给出推荐分、理由与风险提示，并支持通过 API/Web/CLI 将 Top N 复用现有股票分析流水线做深度分析。
- [改进] 市场推荐扩展美股股票/ETF 候选发现：`--recommend-market us` 支持从内置高流动性股票/ETF 池发现候选，配置 Longbridge 时优先用长桥行情增强，未配置或失败时使用 yfinance 兜底；`all` 现在表示当前可支持的 A 股 + 美股。
- [修复] 市场推荐在候选为空时跳过市场统计和板块排行补充；候选已生成但上下文接口失败或超时时改为保留候选并记录备注；候选发现层跳过未实现全市场候选的空数据源，放宽候选源整体超时，并为 AkShare 东财/新浪候选接口分别设置短超时，确保东财异常时能实际尝试新浪行情兜底，减少行情源异常导致 `--recommend` 长时间等待或返回空结果的问题。
